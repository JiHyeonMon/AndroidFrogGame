...
class Game {
    ...
    fun isFrogGetOn(river: River): River? {
        ...
        if (river.crocodile.getLeft() <= frog.getLeft() && frog.getRight() <= river.crocodile.getRight()) {
            ...
            if (river.crocodile.reverse) {
                ...
                } else frogFlowOnRiver = river // 개구리 악어에 잘 올라타서 같이 움직임. 강의 속력과 방향에 맞춰 움직이기 위해 매개변수 설정
